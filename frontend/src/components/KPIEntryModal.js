...
export default function KPIEntryModal({ onClose, onSuccess, editEntry = null }) {
  ...
  const handleSubmit = async () => {
    ...
    // Clients field removed - same as sales
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) { onClose(); } }} className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-4 mb-6">
            ...
            {/* Nombre de clients supprimé - même chose que nombre de ventes */}
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    value={nbProspects}
                    onChange={(e) => setNbProspects(e.target.value)}
                    placeholder="0"
                    step="1"
                    min="0"
                    className="flex-1 px-4 py-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-[#ffd871] focus:border-transparent"
      ...
      {/* Warning Modal */}
      {showWarningModal && (
        <div onClick={(e) => { if (e.target === e.currentTarget) { onClose(); } }} className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-[60]">
