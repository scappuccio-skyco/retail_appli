...
import { Plus, Building2, Users, TrendingUp, BarChart3, Lock, TrendingDown, Minus, MapPin, Download, Loader2, Trash2, Settings, X } from 'lucide-react';
import { exportGerantDashboardPdf } from './gerantDashboardPdfExport';
import { api } from '../../../lib/apiClient';
import { toast } from 'sonner';
import { getPeriodDates, getPeriodNumber, formatPeriod, getPeriodLabel } from './gerantDashboardUtils';
import useGerantBusinessContext from './useGerantBusinessContext';
import GerantBusinessContextModal from './GerantBusinessContextModal';
import GerantDeleteConfirmModal from './GerantDeleteConfirmModal';
...
export default function GerantDashboardView({
  ...
  // ── Business context ───────────────────────────────────────
  const ctx = useGerantBusinessContext();
  ...
  return (
    <div ref={contentRef}>
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => exportGerantDashboardPdf(contentRef, formatPeriod(periodType, periodOffset), setExporting)}
              disabled={exporting}
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4 p-3 bg-gray-50 rounded-xl border border-gray-200">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPeriodOffset(periodOffset - 1)} className="p-1.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-600 rounded-lg transition-all text-xs font-bold">◀</button>
            ...
            <button
              ...
              className={`p-1.5 border rounded-lg transition-all text-xs font-bold ${
                periodOffset >= 0 ? 'bg-gray-100 border-gray-200 text-gray-300 cursor-not-allowed' : 'bg-white border-gray-300 hover:bg-gray-50 text-gray-600'
              }`}
            >▶</button>
          </div>
          {periodOffset !== 0 && (
            <button onClick={() => setPeriodOffset(0)} className="text-xs text-blue-600 hover:text-blue-700 font-semibold underline whitespace-nowrap">
              ↻ Actuel
            </button>
          )}
        </div>
          ...
          <>
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-3 mb-3 px-4 py-3 bg-indigo-50 border border-indigo-200 rounded-xl">
                ...
                <button
                  onClick={() => ctx.openContextModal([...selectedIds], `${selectedIds.size} magasins sélectionnés`)}
                  ...
                  {rankedStores.map((s, index) => {
                    ...
                    return (
                      <tr key={s.id} className={`hover:bg-orange-50/40 transition-colors ${index < 3 ? 'bg-orange-50/20' : 'bg-white'}`}>
                        ...
                        <td className="px-4 py-4 text-center"><span className="text-xl">{rankIcon}</span></td>
                        ...
                        <td className="px-4 py-4 text-right hidden lg:table-cell">
                          {s.periodVentes > 0 && s.periodCA > 0
                            ? <p className="font-semibold text-gray-700">{Math.round(s.periodCA / s.periodVentes).toLocaleString('fr-FR')} €</p>
                            : <span className="text-gray-400 text-sm">—</span>}
                        </td>
                        <td className="px-4 py-4 text-right hidden lg:table-cell">
                          {s.periodProspects > 0 && s.periodVentes > 0
                            ? <p className="font-semibold text-gray-700">{Math.round((s.periodVentes / s.periodProspects) * 100)} %</p>
                            : <span className="text-gray-400 text-sm">—</span>}
                        </td>
                        <td className="px-4 py-4 text-center hidden md:table-cell">
                          {hasEvo ? (
                            <div className="flex flex-col items-center gap-0.5">
                              {evo > 0
                                ? <span className="inline-flex items-center gap-1 text-xs font-bold text-green-700 bg-green-100 px-2 py-1 rounded-full"><TrendingUp className="w-3 h-3" />+{Math.round(evo * 10) / 10}%</span>
                                : evo < 0
                                  ? <span className="inline-flex items-center gap-1 text-xs font-bold text-red-700 bg-red-100 px-2 py-1 rounded-full"><TrendingDown className="w-3 h-3" />{Math.round(evo * 10) / 10}%</span>
                                  : <span className="inline-flex items-center gap-1 text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-full"><Minus className="w-3 h-3" />0%</span>}
                              {s.isPartialComparison && <span className="text-gray-400 text-xs italic">vs J identique</span>}
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              onClick={(e) => { e.stopPropagation(); ctx.openContextModal([s.id], s.name); }}
      ...
      <GerantBusinessContextModal
        contextModal={ctx.contextModal}
        contextForm={ctx.contextForm}
        contextLoading={ctx.contextLoading}
        contextSaving={ctx.contextSaving}
        onClose={ctx.closeContextModal}
        onSave={ctx.handleContextSave}
        toggleClientele={ctx.toggleClientele}
        toggleKpi={ctx.toggleKpi}
        setContextForm={ctx.setContextForm}
      />

      <GerantDeleteConfirmModal
