...
export default function StoreKPIModal({ onClose, onSuccess, initialDate = null }) {
  ...
  const handleOverviewAIAnalysis = async () => {
    ...
    try {
      ...
      // Prepare context based on view mode
      let periodContext = '';
      if (viewMode === 'week') {
        periodContext = `Semaine sélectionnée : ${selectedWeek}`;
      } else if (viewMode === 'month') {
        periodContext = `Mois sélectionné : ${selectedMonth}`;
      } else if (viewMode === 'multi') {
        if (multiPeriod === '3months') periodContext = '3 derniers mois';
        else if (multiPeriod === '6months') periodContext = '6 derniers mois';
        else if (multiPeriod === '12months') periodContext = '12 derniers mois';
      }
      ...
      const res = await axios.post(
        `${API}/api/manager/kpi-analysis`,
        { kpi_data: kpiContext },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setOverviewAIAnalysis(res.data.analysis);
              ...
              {viewMode === 'month' && (
                <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-4 border-2 border-purple-200">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                    ...
                    <button
                      onClick={() => setShowOverviewAIModal(true)}
                      disabled={!historicalData.length || !selectedMonth}
                      className="px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 whitespace-nowrap"
                    >
                      🤖 Analyse IA
                    </button>
