...
/**
 * Composant de formulaire diagnostic partagé (seller + manager).
 * Gère la navigation, le shuffle des options et l'UI.
 * La logique de soumission est déléguée au parent via `onSubmit`.
 *
 * Props:
 *   questions   — array de { section, items: [{ id, text, options }] }
 *   title       — string affiché dans le header
 *   onSubmit    — async ({ responses, allQuestions }) — logique métier du parent
 *   onClose     — callback fermeture
 *   isModal     — boolean (défaut true) : si false, rendu inline sans overlay
 */
export default function DiagnosticFormBase({ questions, title, onSubmit, onClose, isModal = true }) {
  ...
  // Fisher-Yates shuffle, original index préservé pour scoring DISC
  ...
  const handleNext = () => { if (currentIdx < totalQuestions - 1) setCurrentIdx(i => i + 1); };

  const handleSubmit = async () => {
    setLoading(true);
    try {
      await onSubmit({ responses, allQuestions });
    } finally {
      setLoading(false);
    }
  };
  ...
  const inner = (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex-1 p-6 flex flex-col justify-center transition-opacity duration-200 ${animating ? 'opacity-0' : 'opacity-100'}`}>
        <p className="text-gray-800 font-semibold text-lg mb-6 leading-relaxed">{question.text}</p>
