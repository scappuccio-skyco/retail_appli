...
const conflictReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SET_PENDING_RECOMMENDATION':
      return { ...state, pendingRecommendation: action.payload, loading: false };
    case 'SET_AI_RECOMMENDATIONS':
      return { ...state, aiRecommendations: action.payload, pendingRecommendation: null };
    case 'SHOW_RESULT':
...
export default function ConflictResolutionForm({ sellerId, sellerName }) {
  ...
  useEffect(() => {
    if (state.pendingRecommendation) {
      // Séparer les updates d'état pour éviter insertBefore error
      setTimeout(() => {
