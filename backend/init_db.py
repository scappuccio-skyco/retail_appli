...
def init_database():
    ...
    try:
        ...
        # Vérifier si un compte gérant existe (propriétaire d'entreprise)
        gerant_count = db.users.count_documents({"role": "gerant"})
        
        if gerant_count == 0:
            logger.info("🔍 Aucun compte gérant trouvé dans la base de données")
            logger.info("🚀 Création du compte gérant par défaut...")
            ...
            # Créer l'utilisateur gérant (propriétaire d'entreprise)
            admin_user = {
                "id": str(uuid4()),
            ...
            logger.info("✅ Compte gérant créé avec succès !")
            logger.info(f"   📧 Email: {default_admin_email}")
            logger.info(f"   🔑 Mot de passe: {default_admin_password}")
            logger.info(f"   👤 Rôle: gérant (propriétaire d'entreprise)")
