...
**Via l'API** :
```bash
curl -X GET "https://fix-deploy-access.preview.emergentagent.com/api/v1/integrations/my-stores" \
...
DATA: lv_api_key TYPE string VALUE 'rp_live_votre_cle_api_ici',
      lv_url     TYPE string VALUE 'https://fix-deploy-access.preview.emergentagent.com/api/v1/integrations/kpi/sync',
      lv_json    TYPE string,
...
2. **Créer un Receiver Channel** :
   - Type : HTTP
   - URL : `https://fix-deploy-access.preview.emergentagent.com/api/v1/integrations/kpi/sync`
   - Méthode : POST
   - Headers :
     ```
...
**URL** : `POST https://fix-deploy-access.preview.emergentagent.com/api/v1/integrations/kpi/sync`
...
```bash
curl -X POST "https://fix-deploy-access.preview.emergentagent.com/api/v1/integrations/kpi/sync" \
  -H "X-API-Key: rp_live_votre_cle_api_ici" \
