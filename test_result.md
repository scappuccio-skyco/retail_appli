...
agent_communication:
  - agent: "main"
    message: "CONFLICT RESOLUTION FEATURE - BACKEND IMPLEMENTED: Added conflict resolution functionality to help managers resolve conflicts with sellers. Backend includes: 1) ConflictResolution model with structured questions (contexte, comportement_observe, impact, tentatives_precedentes, description_libre) and AI analysis fields, 2) AI function generate_conflict_resolution_analysis() that creates personalized recommendations based on manager profile, seller profile (diagnostic, debriefs, competences, KPIs), 3) POST /api/manager/conflict-resolution endpoint to create conflict resolutions, 4) GET /api/manager/conflict-history/{seller_id} endpoint to retrieve history. Ready for backend testing using manager account."
  - agent: "testing"
