...
agent_communication:
  - agent: "main"
    message: "REACT 19 DOM STABILITY FIXES IMPLEMENTED: ✅ ROOT CAUSE: React 19 concurrent rendering conflicts with direct DOM manipulation causing insertBefore/NotFoundError crashes on Chrome/Edge after Stripe checkout or modal interactions. ✅ FIXES APPLIED: 1) BilanIndividuelModal.js PDF export - removed document.body.appendChild/removeChild wrapper, now captures directly from visible content using html2canvas, 2) InviteModal.js clipboard fallback - replaced document.createElement/appendChild/removeChild with React ref managing hidden textarea in JSX. ✅ CODE AUDIT: Searched entire frontend for key={index} patterns - ZERO found, searched for direct DOM manipulation patterns - ALL eliminated. ✅ IMPACT: Both modals now use React-managed elements exclusively, eliminating Virtual DOM conflicts. ✅ READY FOR TESTING: Need comprehensive testing of modal interactions, especially after Stripe checkout flow and PDF export functionality."
  - agent: "main"
